...
- **⑦ commit 的量詞：個（不用「顆」）——本條首次跨全六週一次收（2026-07-17）**：W1 deck 曾同時存在 顆 23／個 4，**但那 4 個「個」全在骨幹上**——【07】章節扉頁「版本快照：完成第一個 commit」、P3 段標題「Git 身分與第一個 commit」、kit 分組 `>【07】第一個 commit`、以及 **SEO／OG meta**（分享連結的預覽文案）。現況是「**骨幹說個、內文說顆**」，故**統一方向＝讓內文對齊骨幹**；反向要動章節扉頁＋段標題＋kit 分組＋SEO meta，churn 更大且動到脊椎。**「顆」的錨點在 W1 本來就不存在**：它之所以順口是呼應 commit graph 的圓點，但 W1 全段沒有線圖（`git-01` 只剩 `git diff` 一張、`--graph` 頁是一排 `*`）——**圓點是 W2 的語彙**。
  - **同輪修掉「第一顆快照」×4（【15】data-05）→「第一張快照」**：這是把 commit 的量詞套到**快照**上。快照是照片類、量詞是**張**，而 deck 自己的比喻正好在打臉它——【07】寫著「`git commit`：保存到儲存庫（**拍照**）」「用**拍團體照**記住三層架構」。**拍照留下的是一張，不是一顆。**（統一成「個」會矇混過去，但「張」才準且與比喻同源。）
...
- **中文定名再定「技能儀表板」（2026-07-18）**：`Dashboard`→`儀表板`、英文名同步「Skill Dashboard」；`skill-dashboard` repo 名、`W1/dashboard/` 路徑、`DASHBOARD_DATA`／`SKILL DASHBOARD` 識別字全不動（中英 1:1 對譯的結論不變、反而更成立：技能＝skill、儀表板＝dashboard）。全名「技能 Dashboard」→「技能儀表板」、裸「Dashboard」→「儀表板」（含「磚／線／schema」）；**「技能儀表板」後不接「模板」**——p184 QR 頁 heading 收成「技能儀表板」、onboarding 資源卡改「起始檔案」、docs 的「Dashboard 模板」改「儀表板的模板檔案」。範圍＝deck 4 份＋docs＋`W1/dashboard/{data.js,index.html}` 共 126 處＋本檔活段落（決策表 #1/#3/#4、【15】知識點名）；**本節 07-17 及更早的 dated 段落依慣例保留舊名「技能 Dashboard」**。連動：`W1/dashboard/` 本尊在課程模板庫 repo（外部），`index.html` title 與 `data.js` 需另同步。

【15】挑戰完成項＝該塊唯一主線知識點（2026-07-17）：

- **「部署『技能儀表板』頁面」入【15】為非加分項**（kit 的「*整合實做，無新技能點*」行由它取代；兩顆 ⟨加分⟩ 疑難項不動）——儀表板上該分組（W1 的 06 區塊）主線進度條因此**只繫在這一項**：部署完成即 100%。頭條「全課程進度」維持知識點驅動、不另設計分層。
- **與「🔨 整合實做不產生知識點」不衝突**：完成項是**驗收位**、不是新技能——例外規則入 docs/README「知識點清單推導」；W2–W6 的 🔨 塊是否比照，備課時逐塊定。資料端 `milestone: true` 區塊（本週目標）維持純展示、不參與計分。
- **軌跡**：同日曾誤走「頭條 % 改由里程碑驅動」全套（頭條＝已完成里程碑÷已解鎖週、六份 kit 各設「里程碑」節、milestone 磚帶 done 可點亮），當日撤回——原意就是「該塊主線＝這一項」的區塊級語意，不是全課程級。
