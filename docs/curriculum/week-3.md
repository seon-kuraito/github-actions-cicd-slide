...
- [ ] *改檔不 commit → stash → 切分支辦事 → 回來 pop 繼續*
- [ ] >stash 的內容去了哪？
- [ ] >stash 也能帶名字：多筆暫存的管理
...
- [ ] *rm 一個檔案、mv 一個檔案，用 git status 說出差別*
- [ ] >rm --cached：只從追蹤移除
- [ ] >直接在檔案總管刪檔案會怎樣：Git 的視角

## P2 — reset 與 revert（23 頁）
...
- [ ] 【？】退到那一顆，你的檔案跟不跟著退？（reset 有 soft／mixed／hard 三種）
...
- [ ] *hard reset 掉一顆 commit → reflog 找 hash → 救回來*
- [ ] >reflog 的保存期限
...
- [ ] 【？】只想要那條分支的其中一顆 commit，merge 整條會發生什麼事？
- [ ] 【！】整條 commit 全被併進來——你只要一顆，殺雞用牛刀
- [ ] git cherry-pick \<hash\>：把那顆 commit 摘過來
- [ ] cherry-pick 之後：同內容、新 hash（呼應 W2 的 hash 課）
- [ ] 使用情境：hotfix 搬運
- [ ] *把別的分支的一顆 commit 摘到 main，log 確認*
- [ ] >cherry-pick 衝突處理
- [ ] >一次摘多顆：範圍語法 A..B

- [ ] **【38】歷史的剪接台（interactive rebase）**
- [ ] 【？】三顆「WIP」「fix」「再修一下」的 commit，敢給 reviewer 看嗎？
- [ ] 【！】不敢就進剪接台——rebase -i 把 WIP／fix 揉成一顆乾淨 commit（但推過的別動）
- [ ] git rebase -i HEAD~3：進入剪接台
- [ ] 剪接台畫面解剖：pick 清單與指令表
- [ ] squash：把 WIP 揉成一顆好 commit
- [ ] reword：只改訊息
- [ ] drop 與 reorder：刪除與換序
- [ ] *連發三顆爛 commit → rebase -i 整成一顆、訊息重寫*
- [ ] >剪接台裡迷路：--abort 萬用
- [ ] >改寫歷史的紅線：推過的不要動（黃金法則回收）
- [ ] >Squash and merge：合 PR 時 GitHub 把整個 PR 揉成一顆進 main——等於自動幫你做 rebase -i squash（main 上一個 PR 一顆乾淨 commit）
...
- [ ] SemVer：版本號三碼怎麼取
- [ ] *幫自己的技能樹 Dashboard 打 `v1.0.0`（annotated）→ push tag → 發第一個 Release*
- [ ] >tag 打錯了怎麼辦
...
- [ ] *看圖說出一個 feature 從 develop 走到 main 要經過哪幾條分支*
...
- [ ] *把你 Dashboard 的一次改動，畫成 GitHub Flow 路徑（feature→PR→main→上線）*
...
- [ ] *說得出你 Dashboard 現在屬哪一套、為何還用不到環境分支*
- [ ] >W5／W6 伏筆：notify 系統會真的走一次 staging → production

## P6 — 情境挑戰「搞爛再救」＋收進 Dashboard 發版（~24 頁）
...
- [ ] 災情一（手滑）：發一顆好 commit → `reset --hard HEAD~1` 弄丟它 → **reflog** 找回來
- [ ] 災情二（爛歷史）：連發三顆「WIP／fix／再修」→ **rebase -i** squash 成一顆（都還沒 push，安全）
- [ ] 災情三（散落他枝）：在 feature 分支發一顆修復、不合回 → **cherry-pick** 撿回 main
- [ ] 至此 main 乾淨 → push 上遠端
- [ ] 災情四（已公開）：push 一顆有錯的 commit（收不回）→ **revert** 產生反向 commit → 再 push
- [ ] *四種災情都救回，main 剩一條乾淨的 log*
...
- [ ] *Dashboard 技能樹亮了 W3 這批點、Releases 頁有 v1.0.0*
...
- [ ] *把慘況用人話描述給 AI（例：「我 reset --hard 把 commit 弄丟了」）*
- [ ] *AI 開的處方——看懂每一步再執行*
- [ ] *請 AI 建議歷史整理（rebase -i 清單）——一樣看懂再動手*
...
- [ ] *AI 幫你建立本機 Repo（skill: `create-repo`）（升級版：git init＋.gitignore）*
- [ ] *AI 用 gh 建立與綁定 GitHub Repo（skill: `remote-repo`）（升級版，給定模版）*
- [ ] *接著 branch → commit → PR：沿用 W2 的 skills 一口氣跑完（不重教，只重跑）*
