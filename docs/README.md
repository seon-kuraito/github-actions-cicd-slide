...
| — | 8/8 | — | 停課 | | | | |
| 4 | 8/15 | MODULE · TEAMWORK | 團隊協作與 CI 初見 | 像團隊一樣工作、第一個綠勾 | `notify-w4-teamwork` | MCP＋GitHub 串接＋PR 描述＋AI review | ~113 |
...
- **程式碼模板**（規劃暫名「課程模板庫」，deck 定名；2026-07-13 追記；onboarding 資源頁 announced）：單一 repo＝模板發放處（Dashboard、Agent Skills 等）＋官方提問處（發問 Issue 掛此 repo）。與 `my-dashboard`「Use this template」載體的關係（收連結或直接承載）於 W1 kit 製作時定。
...
- **發問管道**：GitHub Issue，掛在程式碼模板 repo（onboarding 資源頁預告、頁 17「遇到問題—開 Issue」announced；用課程教的工具發問，本身即教學）。
