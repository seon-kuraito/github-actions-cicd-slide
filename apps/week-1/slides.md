...
seoMeta:
  ...
  ogDescription: "從零開始學 Git 版本控制、終端機、VS Code、GitHub 與 GitHub Pages，完成第一個 commit、push 遠端並親手部署技能 Dashboard，體驗作品擁有公開網址的那一刻。"
...
items:
  ...
  - { no: P3, heading: Git 身分與第一個 commit, desc: 設定作者署名，開始用 Git 追蹤，拍下第一個版本 }
---
...
layout: content-01
heading: 為什麼需要版本控制？
body: "`report_final.doc`、`report_final_v2.doc`、`report_final_final_v4.doc`⋯⋯每天複製一份資料夾、改壞後找不到舊版、兩個人各改一份==卻不知道怎麼合起來==。這些混亂，都是版本控制要處理的問題。"
...
layout: content-01
heading: 版本控制在解決什麼？
body: "版本控制用三個機制整理變更：==快照==，在重要時間點留下完整狀態；==回溯==，需要時回到指定版本；==協作==，讓多人修改能被追蹤、比對與合併。每一次變更都有紀錄，也更容易說清楚發生了什麼。"
note: "把混亂的手動備份，變成可追蹤、可回復、可合併的工作流程。"
---
...
---
layout: diagram-02
heading: 集中式 vs 分散式版本控制
expected:
  tag: 集中式（SVN 時代）
  label: 只有一台中央伺服器保存歷史，斷線就不能提交、伺服器掛了全隊停擺
  diagramLabel: 集中式
  diagramNote: "820 × 440 · 示意圖佔位"
actual:
  tag: 分散式（Git）
  label: 每個人本機都有一份完整的歷史，離線照常記錄，遠端只是拿來同步的一份
  diagramLabel: 分散式
  diagramNote: "820 × 440 · 示意圖佔位"
---
...
body: 第一次執行 `git` 時，如果 macOS 跳出「需要安裝命令列開發者工具」，按「安裝」並等它完成即可，Git 會一起補上。若你習慣用 Homebrew，`brew install git` 通常會裝到==比系統內建更新==的版本。
...
layout: content-01
heading: 終端機是什麼？
body: 終端機是一個用==文字指令操作電腦==的視窗，也就是 CLI（Command Line Interface）。GUI 靠滑鼠點按鈕，CLI 靠鍵盤輸入指令；今天的 Git、之後的開發工具與 AI 工具，都會大量在這裡操作。
...
file: git --version
lines:
  - { type: comment, text: "# 確認終端機找得到 Git 指令" }
...
imageLabel: SCREENSHOT
imageNote: "VS Code 下載頁 · 截圖佔位"
---

<!-- p58 -->
---
layout: image-01
slug: vscode/open-folder
heading: 使用 VS Code 開啟資料夾
body: "「File → Open Folder」（或把資料夾拖進 VS Code）選擇剛剛建立的 `hello-git`，VS Code 左側的「檔案總管」就會列出裡面的檔案——它和 Windows 的檔案總管同名，但只顯示你開啟的這個資料夾。往後我們都以「開啟一個資料夾」為單位在工作。"
imageLabel: SCREENSHOT
imageNote: "VS Code 開啟資料夾 · 截圖佔位"
---
...
---
layout: image-01
slug: vscode/code-command
heading: 使用 VS Code 開啟目前資料夾
body: "macOS 在 VS Code 的命令面板選擇「在 PATH 中安裝 'code' 命令」；Windows 則是在安裝時勾選「加入 PATH」。之後在終端機輸入 `code .`，就能用 VS Code 開啟目前資料夾。"
imageLabel: SCREENSHOT
imageNote: "VS Code 命令面板 · 截圖佔位"
---
...
body: "按下 Ctrl / Control + `（反引號），或從上方選單選擇「終端機 → 新增終端」，VS Code 會在底部開啟一個終端機。編輯器與終端機在同一個視窗，就不用再切來切去。"
imageLabel: SCREENSHOT
imageNote: "VS Code 整合終端 · 截圖佔位"
---
...
layout: qa-02
label: git/identity
heading: 這是版本紀錄裡的作者署名，不是 GitHub 登入
sub: Git 會用這組名字與 email，標記之後留下的每一個版本
---
...
body: "普通資料夾加上 `.git`，Git 就能在這==裡保存版本紀錄==，這樣的資料夾就叫做「儲存庫」（repository，簡稱 repo）。既然這裡已經交給 Git 管理，下一步就可以用 `git status` 問它目前看到什麼。"
...
layout: content-01
heading: 找出被隱藏的 .git
body: "`.git` 是==隱藏資料夾==：macOS 可在 Finder 按 cmd + shift + . 顯示隱藏檔；Windows 則是在檔案總管的「檢視」勾選「隱藏的項目」。如果是在 VS Code 側邊欄找不到，多半是它預設用 `files.exclude` 把 `.git` 藏起來。"
note: "看不到 .git，不代表 git init 失敗，先確認目前工具有沒有顯示隱藏檔。"
---
...
body: "剛剛 `git status` 看到的三層，可以想成拍一張團體照：==工作目錄==是大家還在走位、檔案還在修改；==暫存區==是選好這次要入鏡的人，也就是用 `git add` 挑出要提交的變更；==儲存庫==則是在按下 `git commit` 後，真正被保存下來的那張照片。"
note: "先修改，再挑選，最後保存——這就是工作目錄、暫存區、儲存庫的順序。"
---

<!-- p90 -->
---
layout: git-areas
heading: git add：放進暫存區（入鏡構圖）
command: git add
meta: 工作目錄 → 暫存區 → 儲存庫
areas:
  - name: WORKING
...
items:
  ...
  - { no: P6, brand: github, heading: Dashboard 公開, desc: 套用範本、更新資料，公開自己的技能頁面 }
...
note: "好的 commit 不是越大越好，而是每一個都能「清楚說明目的」。"
---
...
areas:
  ...
  - name: REPOSITORY
    state: solid
    items:
      - { flag: c3d4e5f, flagTone: brand, text: 調整版面配色 }
arrows:
...
note: "add 不是開啟自動追蹤，而是把當下這一版放進暫存區。"
---
...
imageLabel: SCREENSHOT
imageNote: "「Settings → Pages 設定畫面」 · 截圖佔位"
---
...
heading: Step · ==01== / 09
steps:
  - { no: "01", heading: 建立專案資料夾, desc: 建立新的資料夾，先把 Dashboard 專案獨立出來, state: now }
  - { no: "02", heading: 初始化儲存庫, desc: 讓 Git 接手記錄版本，確認這裡就是新的儲存庫, state: todo }
  - { no: "03", heading: 下載模板, desc: 下載模板檔案，把 index.html 和 data.json 搬進專案, state: todo }
  - { no: "04", heading: 編輯資料, desc: 在 data.json 填入姓名與技能，讓 Dashboard 顯示你的內容, state: todo }
---
  ...
  - name: you/
    children:
      ...
      - name: skill-dashboard/
        ...
        badge: NEW
terminal:
  - { type: comment, text: "# 先為技能 Dashboard 開一個獨立資料夾" }
...
heading: Step · ==02== / 09
steps:
  - { no: "01", heading: 建立專案資料夾, desc: 建立新的資料夾，先把 Dashboard 專案獨立出來, state: done }
...
steps:
  ...
  - { no: "03", heading: 下載模板, desc: 下載模板檔案，把 index.html 和 data.json 搬進專案, state: now }
  - { no: "04", heading: 編輯資料, desc: 在 data.json 填入姓名與技能，讓 Dashboard 顯示你的內容, state: todo }
...
heading: 下載模板
body: 從 GitHub 下載課程模板裡的 `index.html` 和 `data.json`，放進剛剛建立的 `skill-dashboard` 資料夾。放好後，網頁檔案和資料檔案就都準備好了。
imageLabel: SCREENSHOT
imageNote: "「Code → Download ZIP」 · 截圖佔位"
---
...
heading: Step · ==04== / 09
steps:
  - { no: "03", heading: 下載模板, desc: 下載模板檔案，把 index.html 和 data.json 搬進專案, state: done }
  - { no: "04", heading: 編輯資料, desc: 在 data.json 填入姓名與技能，讓 Dashboard 顯示你的內容, state: now }
  - { no: "05", heading: 提交變更, desc: add 後 commit 留下第一張快照，作為專案歷史起點, state: todo }
...
heading: 編輯資料
body: 用 VS Code 打開 `skill-dashboard`，編輯 `data.json`。把名字改成你的姓名，並在資料裡點亮目前為止學過的技能（例如 Git、commit、push），完成後記得存檔。
imageLabel: SCREENSHOT
imageNote: "VS Code 編輯 data.json · 截圖佔位"
---
...
heading: Step · ==05== / 09
steps:
  - { no: "04", heading: 編輯資料, desc: 在 data.json 填入姓名與技能，讓 Dashboard 顯示你的內容, state: done }
...
terminal:
  ...
  - { type: comment, text: "# 這是 Dashboard 專案的起點，之後的修改都會接在這個 commit 後面", accent: true }
---
...
steps:
  ...
  - { no: "08", heading: 設定 GitHub Pages, desc: 在 Settings 指定分支與路徑，啟用 Pages 部署流程, state: todo }
  - { no: "09", heading: 瀏覽公開頁面, desc: 等部署完成後打開網址，確認 Dashboard 已公開, state: todo }
---
...
body: "進到儲存庫的「Settings → Pages」。Source 選擇「Deploy from a branch」，分支選 `main`，資料夾選 `/ (root)`，最後按 Save。設定完成後，GitHub 會開始部署你的 Dashboard。"
imageLabel: SCREENSHOT
imageNote: "「Settings → Pages」 · 截圖佔位"
---
...
steps:
  ...
  - { no: "09", heading: 瀏覽公開頁面, desc: 等部署完成後打開網址，確認 Dashboard 已公開, state: now }
---
...
heading: 瀏覽公開頁面
body: 設定完成後，點擊 GitHub 產生的公開網址。只要頁面正常顯示，就代表你的 Dashboard 頁面已經公開。這就是今天可以分享出去的成果 🎉
imageLabel: SCREENSHOT
imageNote: "公開的 Dashboard 頁面 · 截圖佔位"
---
...
heading: 【15】章節實作
command: '$ hands-on'
sub: '打開 GitHub Pages 頁面，確認你的技能 Dashboard 已經公開'
---
...
heading: 【15】章節補充
command: '# optional'
sub: 如果 Dashboard 沒出現，先從部署設定和資料格式檢查起
---
...
class: brand-github
heading: Pages 沒出現畫面？
body: 常見原因有三：==分支選錯==，要選有 `index.html` 的那條（通常是 `main`）；==路徑不對==，資料夾要選 `/ (root)`；==還在部署中==，Actions 可能還沒跑完。到儲存庫的 Actions 分頁，可以看到目前部署狀態。
note: "先檢查分支、路徑、部署狀態，通常就能找到 Pages 沒畫面的原因。"
---
...
layout: content-01
class: brand-github
heading: data.json 改壞了？
body: 如果畫面空白或跳出錯誤，有可能是因為 JSON 格式壞掉，例如：多一個或少一個逗號、引號沒有成對、括號沒有收好。VS Code 通常會用==紅色波浪線標出位置==，按照提示修好、存檔、commit，然後再開啟一次 `index.html` 確認。
note: "JSON 很嚴格，少一個逗號或引號，都可能讓整個畫面讀不到資料。"
---
...
heading: 【17】章節補充
command: '# optional'
sub: 看懂 skill 的放置層級，知道同一套規則會在哪些專案生效
---
...
class: brand-ai
heading: skill 裝在哪一層？
body: 放在「User Global」（你的使用者目錄）的 skill，會對你的==所有專案生效==；放進專案內的 `.agents/skills`，則只會在==這個專案生效==。今天先採用專案層，讓 create-commit 只服務這份 Dashboard 練習。
...
layout: story-01
kicker: 第一天 · 收工前
heading: 你的第一個作品，已經公開 🎉
sub: 從零開始到公開網址，收工前回顧今天完成的關鍵步驟。
---
...
items:
  ...
  - { no: "06", brand: github, text: "已部署自己的技能 Dashboard，第一批技能正式點亮" }
...
layout: content-01
heading: "下週預告：==分支與衝突=="
body: 下週會把今天公開的 Dashboard 帶進多人開發情境：建立分支、合併變更、處理衝突，最後送出你的第一個 Pull Request。
