...
seoMeta:
  ...
  ogDescription: "六週實作課程，從 Git 版本控制、GitHub 協作與 GitHub Actions CI/CD，一路走到 Docker 部署、通知系統與 AI 協作。從這裡掌握課程設計、技能儀表板與每週重點。"
...
heading: 為什麼是 Git？
layers:
  - { tag: STEP 01, name: Git 版本控制, desc: 這是整套流程的起點——接下來六週的每一步，都會從這裡開始, active: true }
  - { tag: STEP 02, name: GitHub 協作與 CI/CD, desc: PR、code review、自動測試與部署，最後都靠 commit 串起來 }
  - { tag: STEP 03, name: AI 協作, desc: AI 幫你改的每一步，都需要能追蹤、能驗收、也能退回 }
---
...
heading: "技能儀表板：\n第一個上線作品"
body: 第一週會從模板建立自己的版本，部署到 GitHub Pages 公開上線。之後每學會一個知識點，就在技能儀表板上點亮一格；未來第二、三週的分支、衝突與救援技巧，也會直接用在這個作品上。
...
items:
  - { text: "**課程順序重排** — GitHub 與遠端操作移到第一週，CI/CD 概念前移到第四週。" }
...
cards:
  ...
  - { tag: TEMPLATES, name: 程式碼模板, desc: "技能儀表板的起始檔案會從這裡發放，課後提問也在這裡開 GitHub Issue\n\n", url: github-actions-cicd/templates }
...
body: 六週簡報集中在這個課程網站，作為==課堂主線與課後複習==的固定入口；已解鎖的週次可隨時回來查看，包含當週目標、操作步驟、練習任務與延伸提醒，之後查指令、複習觀念或確認作業方向也能回到同一個地方。
...
body: 不管是操作卡住、觀念想不明白，或想把今天內容套到工作情境，都==先把背景說清楚==；包含遇到的情境、已經試過的做法、卡住的地方與想確認的問題，線索越完整，同學和老師越能釐清問題。
...
---
layout: content-01
heading: AI 協作資源：上課時一起使用
body: 課程會發放一些給 AI 協作使用的==輔助資源==，先不用急著理解每個檔案的細節，等每週進到實作任務時，會一起看它們怎麼幫你整理需求、檢查改動、產出紀錄，讓 AI 更貼近當下的專案脈絡。
note: "使用方式：先知道入口在哪裡，實際用法會跟著每週任務示範"
---
...
body: 接著進入第一週簡報，先理解版本控制為什麼重要，再建立自己的技能儀表板，並在今天結束前把它公開上線。
---
